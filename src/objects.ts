...
export function isCorrect(question: Question, answer: string): boolean {
    const fixedAnswer = answer.toLowerCase().trim();
    const expected = question.expected.toLowerCase().trim();
    return fixedAnswer === expected;
}
...
export function isValid(question: Question, answer: string): boolean {
    ...
    if (question.type === "short_answer_question") {
        ...
        const validChoice = question.options.map((Option) => Option.trim());
        return validChoice.includes(answer);
    }
}
...
export function toShortForm(question: Question): string {
    ...
    const trimmedName = name.length < 10 ? name : name.slice(0, 10);
...
export function toMarkdown(question: Question): string {
    //first see if we need to add the multiple choice
    if (question.type === "multiple_choice_question") {
        const markOptions = question.options
            .map((option) => `- ${option}`)
            .join("\n");
